...
def all_movies():
    with open("tv_report-filter.json") as json_file:
...
def genres():
    try:
        with open('tv_report-genres.json') as json_file:
...
def providers(countries):
    with open("tv_report-all-data-providers.json") as json_file:
        provider_data = json.load(json_file)
    result = "------ PROVIDERS ------\n"
    for country in countries:
        result = result + f"\n___{country}___\n"
        filename = f"tv_report-providers-{country}.json"
...
def certifications(countries):
    ...
    for country in countries:
        result = result + f"\n___{country}___\n"
        filename = f"tv_report-certifications-{country}.json"
        with open(filename) as json_file:
            file_object = json.load(json_file)
